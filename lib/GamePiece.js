module.exports = class GamePiece {
  ...
  moveSnake(arrowKey) {
    ...
    if (arrowKey === 'ArrowRight' || this.direction === 'right') {
      snakeHead[0] = snakeHead[0] + 10; 
      this.direction = 'right';
      this.snakeGenerate(snakeHead);
    } else if (arrowKey === 'ArrowLeft' || this.direction === 'left') {
      snakeHead[0] = snakeHead[0] - 10;      
      this.direction = 'left';
      this.snakeGenerate(snakeHead); 
    } else if (arrowKey === 'ArrowDown' || this.direction === 'down') {
      snakeHead[1] = snakeHead[1] + 10; 
      this.direction = 'down';      
      this.snakeGenerate(snakeHead);
    } else if (arrowKey === 'ArrowUp' || this.direction === 'up') {
      snakeHead[1] = snakeHead[1] - 10; 
      this.direction = 'up';
      this.snakeGenerate(snakeHead);
    } 
